...
[3축 감성 태그 산출 기준]
사용자 selections(아티스트·영화·스타일)과 moods(감성 태그)를 조합해 아래 기준으로 각 축 값을 결정하세요.
...
export function buildUserPrompt(req: InferenceRequest): string {
  ...
  if (req.domain === "music") {
    const artists = req.selections.map((s) => `"${s.name}"`).join(", ");
    const genres = Array.from(new Set(req.selections.flatMap((s) => s.genres))).join(", ");
    return [`음악 취향 — 아티스트: ${artists}`, `장르: ${genres}`, `감성 태그: ${moods}`].join(
      " / "
    );
  }
